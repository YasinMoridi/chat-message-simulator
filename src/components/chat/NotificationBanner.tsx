...
export const NotificationBanner = ({
  ...
  return (
    <div
      aria-hidden="true"
      className={cn(
        "pointer-events-none absolute left-0 right-0 top-0 z-30 flex justify-center px-2.5 pt-2.5 transition-all duration-300 ease-out",
        visible ? "translate-y-0 opacity-100" : "-translate-y-8 opacity-0",
      )}
    >
      {platform === "ios" ? (
        <div className="flex w-full max-w-[calc(100%-8px)] items-start gap-2.5 rounded-2xl bg-white/85 px-3 py-2.5 shadow-[0_4px_18px_rgba(0,0,0,0.18)] backdrop-blur-md">
          {icon}
        ...
        <div className="flex w-full max-w-[calc(100%-8px)] items-center gap-2.5 rounded-xl bg-slate-900/90 px-3 py-2 shadow-[0_4px_16px_rgba(0,0,0,0.25)]">
          {icon}
          <div className="min-w-0 flex-1">
